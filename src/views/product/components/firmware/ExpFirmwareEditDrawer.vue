    ...
    <div v-if="localFirmware" class="drawer-content">
      <div class="info-card mb-6">
        <div class="header-row">
          ...
          <el-tag
            :type="localFirmware.verified ? 'success' : 'warning'"
            effect="dark"
            size="small"
            round
          >
        ...
        <div class="meta-grid">
          ...
          <div class="meta-item">
            ...
            <div class="value-row">
              <span class="font-mono text-xs">{{ displayKey }}</span>
              <el-icon
                v-if="displayKey !== 'N/A'"
                class="copy-btn"
                @click="copyText(displayKey)"
                title="复制 Key"
              >
                <CopyDocument />
              </el-icon>
...
// 计算属性：解析 Firmware Key
const displayKey = computed(() => {
  if (!localFirmware.value) return 'N/A'
  const fw = localFirmware.value as any
  return (
    fw.firmwareKey ||
    fw.FirmwareKey ||
    fw.Key ||
